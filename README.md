...
See [`docs/geometry.md`](docs/geometry.md) for the full derivation.
...
The sweep self-consistency figure is the one worth explaining. A 180° sweep returns the scan
plane to where it started, mirrored, so the last slice must equal the first reflected about the
pole. Scoring that agreement measures the scanner's own angular error **without reference to
the scene**, and simultaneously proves the drive didn't lose steps.
...
| Printed parts | Yoke, cradle, gears, base, hard stops | 10 |
...
100 µF electrolytic across VMOT, and setting the current limit with the motor attached.

Full detail in [`docs/wiring.md`](docs/wiring.md).
...
* Print gears flat on the bed, teeth up, 4 perimeters, no supports. Never on edge.
...
* **Balance the assembly about the tilt axis.** An unbalanced load applies a moment that
  reverses sign through neutral, driving the gear teeth against alternate flanks and putting a
  discontinuity through the middle of every scan.
* Two hard stops, 180° apart. No endstop switch is needed — see homing below.

STLs in [`cad/stl/`](cad/stl), STEP and Inventor sources alongside.
Assembly notes in [`docs/assembly.md`](docs/assembly.md).

---

## Usage

Upload `firmware/tilt_axis/tilt_axis.ino`, then in the Serial Monitor at 115200 with Newline
line endings:
...
* **Room comes out mirrored** — add `--mirror`. YDLIDAR bearings increase clockwise, which
  ...
  property of a given build; determine it once against an asymmetric feature.
...
Three things have to be recovered before the cloud means anything, and the approach to each is
...
mirror-consistency test described above, which is independent of the scene — an earlier method
based on range invariance at the pole was abandoned after it confidently locked onto a yoke arm
occluding the beam, which produces an identical signature.
...
**Handedness.** As above — one flag, fixed per build.

Calibrate on a clean scene. A cluttered room supplies cabinet doors and appliances as
"planes", and the solve gets pulled in inconsistent directions.
...
software/x2l.py        YDLIDAR X2L driver, protocol from the datasheet
software/scan3d.py     capture, calibration, reconstruction, measurement
cad/stl/               printable parts
cad/step/  cad/source/ editable CAD
docs/                  geometry, wiring, calibration, assembly
